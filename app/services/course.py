...
from ..schemas import CourseType, CourseCreate, CourseUpdate, UserCourseCreate, CourseResponse
...
class CourseService:
    ...
    async def list_course(self, skip: int, limit: int):
        total_course, list_course = crud_course.list_course(db=self.db, skip=skip, limit=limit)
        list_course = [CourseResponse.from_orm(item) for item in list_course]
